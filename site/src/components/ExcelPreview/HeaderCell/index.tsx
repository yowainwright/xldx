...
export function HeaderCell({ columnLetter, header, editable = true, onChange }: HeaderCellProps) {
  ...
  return (
    <TableHead className="sticky top-0 z-10 min-w-[100px] border-r border-border bg-muted text-center">
      <div className="mb-0.5 text-[10px] text-muted-foreground/60">{columnLetter}</div>
      {showEditor ? (
        <EditableHeader value={header} onChange={handleChange} onBlur={handleBlur} />
      ) : (
        <div
          onClick={handleClick}
